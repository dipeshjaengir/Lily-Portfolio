...
const Paintings = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-warm-white text-gallery-dark pt-32 pb-24 px-6 md:px-12 relative">
      <Helmet>
      ...
      <header className="max-w-7xl mx-auto mb-16 md:mb-24 text-left">
        ...
        {/* Filter Navigation */}
        <div className="flex gap-6 mt-12 border-b border-black/5 pb-4 text-[10px] font-sans tracking-[0.2em] uppercase font-medium">
          {subMediums.map(med => (
            <button
              key={med}
              onClick={() => setFilterMedium(med)}
              className={`pb-1 transition-all relative cursor-pointer ${
                filterMedium === med ? 'text-gallery-dark' : 'text-gallery-dark/40 hover:text-gallery-dark/70'
              }`}
            >
              {med}
              {filterMedium === med && (
                <motion.div 
                  ...
                  className="absolute bottom-[-5px] left-0 w-full h-[1.5px] bg-gallery-dark"
                />
              )}
            </button>
          ))}
          ...
          {filteredPaintings.map((art, index) => {
            ...
            return (
              <motion.div
                ...
                className="break-inside-avoid bg-white/80 border border-black/5 p-4 md:p-5 shadow-sm hover:shadow-premium transition-shadow duration-500 group flex flex-col relative"
              >
                {/* Image frame */}
                <div 
                  onClick={() => setActiveArtwork(art)}
                  className="relative overflow-hidden w-full bg-soft-white border border-black/[0.04] mb-5 flex items-center justify-center p-1 cursor-zoom-in"
                >
                  ...
                  <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 pointer-events-none">
                    <button
                      ...
                      className="p-2 rounded-full bg-warm-white text-gallery-dark shadow hover:bg-neutral-50 cursor-pointer pointer-events-auto"
                  ...
                  <div className="relative w-full h-full flex items-center justify-center">
                    {art.status === 'Sold' && (
                      <div className="absolute top-3 left-3 z-10 bg-neutral-900/95 text-[8px] font-sans tracking-[0.25em] text-white uppercase py-1 px-2.5 backdrop-blur-xs select-none">
                        SOLD
                      </div>
                    )}
                    <Image
                ...
                <div className="flex flex-col text-left px-1">
                  <div className="flex justify-between items-baseline mb-1">
                    <h3 className="font-serif text-base font-light tracking-wide text-gallery-dark truncate pr-2">
                      {art.title}
                    </h3>
                    {art.year && (
                  ...
                  <div className="flex justify-between items-center pt-2.5 border-t border-black/5 text-[9px] font-sans tracking-widest uppercase font-medium">
                    {art.status && (
                      <span className={`flex items-center gap-1.5 ${
                        art.status === 'Available' || art.status === 'For Sale' ? 'text-neutral-900 font-semibold' : 'text-neutral-400'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${
                          art.status === 'Available' || art.status === 'For Sale' ? 'bg-neutral-900' : 'bg-neutral-300'
                        }`} />
                        {art.status}
                      </span>
                    )}
                    ...
                    <Link
                      to={`/artwork/${art.id}`}
                      className="text-neutral-400 hover:text-neutral-900 transition-colors ml-auto"
                    >
                      DETAILS &rarr;
                    </Link>
                  </div>
