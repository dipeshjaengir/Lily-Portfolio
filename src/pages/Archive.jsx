    ...
    <div className="w-full min-h-screen bg-warm-white text-gallery-dark pt-32 pb-24 px-6 md:px-12">
      ...
      <section className="max-w-7xl mx-auto">
        {archiveArtworks.length === 0 ? (
          <div className="py-24 text-center border-t border-black/5 font-sans">
            ...
            {archiveArtworks.map((art, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="break-inside-avoid bg-white border border-black/5 p-4 flex flex-col justify-between hover:shadow-premium transition-shadow duration-500 group relative"
                >
                  {/* Image */}
                  <div 
                    onClick={() => setActiveArtwork(art)}
                    className="relative overflow-hidden bg-neutral-50 mb-4 border border-black/5 cursor-zoom-in flex items-center justify-center p-1"
                  >
                    {art.status === 'Sold' && (
                    ...
                    />
                    <div className="absolute top-3 right-3 p-1.5 rounded-full bg-white/90 text-gallery-dark shadow opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                      <Maximize2 size={10} />
                    </div>
                  </div>
                  ...
                  <div className="text-left font-sans text-xs pt-1">
                    {art.title && (
                      <div className="flex justify-between items-baseline mb-1">
                        <h3 className="font-serif text-base font-light tracking-wide truncate pr-2">
                          {art.title}
                        </h3>
                    ...
                    <div className="pt-2.5 border-t border-black/5 flex justify-between items-center text-[9px] tracking-widest uppercase">
                      {art.status && (
